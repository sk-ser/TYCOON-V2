// Script by 𝙰𝙼𝙴𝙴𝙽
//𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁 | 𝙳𝙾𝙽'𝚃 𝙲𝙷𝙰𝙽𝙶𝙴 𝙲𝚁𝙴𝙳𝙸𝚃𝚂🙂
//©Ameen Bot 
const {
...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`917994909899@s.whatsapp.net`]
...
/*const reply = (teks) => {
			xeon.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/ 917994909899?text=hello`}}})
		}*/
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `ameen Bot Ofc.`, "footerText": "*_ 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":` 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁 😎`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*ameen*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             xeon.relayWAMessage(res)
        }
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `ameen`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/ 917994909899?text=hello`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\n 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁😎. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/ 917994909899`,
                "mediaType": "10",
                "mediaUrl": `wa.me/ 917994909899`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/ 917994909899`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : " 917994909899@s.whatsapp.net";
...
*Thank you for registering in the  𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁😎 Database*
...
*「  𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'𝙰𝙼𝙴𝙴𝙽 𝚂𝙴𝚁', imageMessage: imageMsg,
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered  𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁*`)
		}, 2000)
        break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =` ▢ 𝐇𝐢 ${pushname} \n ▢ 𝐆𝐫𝐨𝐮𝐩 𝐂𝐡𝐚𝐭𝐬 : ${groups.length} \n ▢ 𝐏𝐫𝐢𝐯𝐚𝐭𝐞 𝐂𝐡𝐚𝐭𝐬 : ${privat.length}\n ▢ 𝐓𝐨𝐭𝐚𝐥 𝐂𝐡𝐚𝐭𝐬 : ${totalChat.length} \n ▢ 𝐒𝐩𝐞𝐞𝐝 : ${latensie.toFixed(4)} _Second_ \n ▢ 𝐀𝐜𝐭𝐢𝐯𝐞 : ${runtime(process.uptime())} \n ▢ 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦 : ${os.platform()}`
               ...
               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: ` 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = " 917994909899@s.whatsapp.net"
        dtod = ' 917994909899'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
│
│  ❑ *ɢɪᴛʜᴜʙ ʟɪɴᴋ* :
│  https://github.com/Ameen-SER/TYCOON-V2
│  
│  ❑ *ɪɴsᴛᴀ* :
...
case 'bugreport':
              ...
              xeon.sendMessage(' 917994909899@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
`
sendButMessage(from, menu, ` 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁`, [
          {
            buttonId: `command`,
            buttonText: {
...
break
//fun features by ameen
                case 'uglycheck':
					// source code by ameen⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:' 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, ` 𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「  𝙰𝙼𝙴𝙴𝙽 𝚂𝙸𝚁 BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
ameenx = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideosearch?query=${c}&apikey=${HunterApi}`)
reply(mess.wait)
ameenx = ameenx.result
ini_txt = ""
for (var x of ameenx) {
...
case 'writelist':
  ...
  ▢ ${prefix}write ameen
  ▢ ${prefix}folioright ameen
  ▢ ${prefix}folioleft ameen
  ▢ ${prefix}writedown ameen
  ▢ ${prefix}writeleft ameen`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'waifu':
  ...
  break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} ameen`)
